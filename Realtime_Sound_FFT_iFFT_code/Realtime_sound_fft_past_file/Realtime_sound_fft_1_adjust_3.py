...
class realtimesound(object):
    def __init__(self):
        ...
        #pyaudio stuff
        self.RATE =  48000              # samples per second , 192000, 96000Hz
        ...
        self.t = np.array(np.linspace(0, self.SECOND_PER_FRAME, self.CHUNK)) # 길이는 CHUNK
        self.halft = np.array(np.linspace(0, self.SECOND_PER_FRAME, self.CHUNK//2)) #길이는 CHUNK/2
        self.f = np.array(np.arange(0, self.RATE, 1/self.SECOND_PER_FRAME ))  # len(frequencty) = CHUNK
        self.halff = np.array(np.arange(0, self.RATE//2, 1/(self.SECOND_PER_FRAME)))  # len(frequencty) = CHUNK/2
    ...
    def set_plotdata(self,name,data_x,data_y):
        if name in self.traces:
            ...
            if name == 'waveform':
                ...
                self.waveform.setYRange(-500, 255+255, padding=0)
            ...
            if name == 'spectrum':
                ...
                self.spectrum.setXRange(0, self.RATE//2, padding=0.01)   # padding의 의미는?
    ...
    def update(self):
        ...
        wf_data = np.array(wf_data, dtype='b')[::2]      # wf_data길이는 CHUNK
        self.set_plotdata(name='waveform', data_x=self.t, data_y=wf_data)

        sig_fft = fftpack.fft(wf_data)
        power = np.abs(sig_fft)
        sample_freq = fftpack.fftfreq(wf_data.size, d=1//self.RATE)



        sp_data = scipy.fft(np.array(wf_data, dtype='int16') - 128)
        # sp_data = scipy.fft(np.array(wf_data) - 128)

        sp_data = np.abs(sp_data[0:int(self.CHUNK)]) * 2 / (128 * self.CHUNK)  # 절반만 사용하자. 나머지 절반은 필요 없다. 
        sp_data[int(self.CHUNK)//2:] = 0 
        sp_data[0:2]=0
        self.set_plotdata(name='spectrum', data_x=self.f, data_y=sp_data)


        iff_sp_data = scipy.fft.ifft(sp_data)
        iff_sp_data = iff_sp_data[0:int(self.CHUNK)//2]
        
        self.set_plotdata(name='ifftwaveform', data_x=self.halft, data_y=np.real(iff_sp_data))

        return print(len(sp_data), len(iff_sp_data))
