...
class PLot2D(object):
    def __init__(self):
        ...
        self.halfx = np.array(np.linspace(0, self.SECOND_PER_FRAME, self.CHUNK//2)) # len(t) = CHUNK
        self.f = np.array(np.arange(0, self.RATE, 1/self.SECOND_PER_FRAME ))  # len(frequencty) = CHUNK, 
        self.halff = np.array(np.arange(0, self.RATE//2, 1/(self.SECOND_PER_FRAME)))  # len(frequencty) = CHUNK, 간격은 그대로 
    ...
    def update(self):
        ...
        wf_data = np.array(wf_data, dtype='b')[::2] + 128        # wf_data길이는 CHUNK
        ...
        self.set_plotdata(name='ifftwaveform', data_x=self.x, data_y=np.abs(iff_sp_data))
