...
class ultrasonic(object):
    def __init__(self, RATE = 192000 , BLOCKING_LOW_FREQUENCY = 0, BLOCKING_HIGH_FREQUENCY = 0, SHIFT_FREQUENCY = 0):
        ...
        # self.half_time = np.array(np.linspace(0, self.SECOND_PER_FRAME, self.CHUNK//2)) # not use 
    ...
    def set_plotdata(self, name, data_x, data_y):
        if name in self.traces:
            ...
            if name == 'waveform':
                ...
                self.traces[name] = self.waveform.plot(pen=waveform_pen) 
            ...
            if name == 'spectrum':
                ...
                self.traces[name] = self.spectrum.plot(pen=spectrum_pen)
            ...
            if name == 'manipulation_spectrum':
                ...
                self.traces[name] = self.manipulation_spectrum.plot(pen=manipulation_spectrum_pen)
            ...
            if name == 'add_sinewave_spectrum':
                ...
                self.traces[name] = self.add_sinewave_spectrum.plot(pen=add_sinewave_spectrum_pen)
                # self.manipulation_spectrum.setLogMode(x=True, y= True)        
                # # 로그 스케일로 axis 표현할때 
            ...
            if name == 're_waveform':
                ...
                self.traces[name] = self.re_waveform.plot(pen=re_waveform_pen)  # pen is color
    ...
    def update(self):
        ...
        sinewave_volume = 1  
        # volume range = 1 ~ 0
        sinewave_shift_frequency = np.sin(2*np.pi*self.SHIFT_FREQUENCY*np.linspace(0, self.SECOND_PER_FRAME, self.CHUNK)) * 125 * sinewave_volume
        ...
        self.set_plotdata(name='re_waveform', data_x=self.time, data_y=re_wp_data_real_add_sinwave)
        # x axis = time, y axis = re_wp_data_add_sinwave  updated

        ##################### Output the Manipulated IFFT data #######################
        # output = re_wp_data_real_add_sinwave.copy()   # sine + mfft 결과
        # 데이터 크기 범위는 127 ~ -128 이다. 
        ...
        final_output = re_wp_data_real_add_sinwave.copy()
