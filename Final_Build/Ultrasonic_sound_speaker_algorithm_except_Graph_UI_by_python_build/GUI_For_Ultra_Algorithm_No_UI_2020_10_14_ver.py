...
class Ultrasonic_sound_GUI(QWidget):
    ...
    def __init__(self):
        self.app = QApplication(sys.argv)    # 단독으로 실행할 시 이것을 없에 줘야한다.  모듈로 쓸땐 활성화시켜준다
        super().__init__()
        self.initUI()
    ...
    def initUI(self):
        ...
        self.btn_OUT_INDEX = QPushButton('Input Device index set', self)
        ...
        self.showing_Timer = 0  #  GUI가 켜지고 timer를 시작한다. 
    ...
    def gui_Timer(self):         # timer 로 흐르는 시간을 측정한다. 
        ...
        if self.showing_Timer > self.showing_Timer_time_out:
            ...
            # print(self.showing_Timer_time_reverse_counter)
    ...
    def mouseMoveEvent(self, event):
        ...
        # self.status_bar.showMessage(mouse_pt), print(mouse_pt)  ## 상태바에 정보가 표시되게 한다.
        # print(mouse_pt)
        if mouse_pt != None:
            if self.showing_Timer <= self.showing_Timer_time_out:
                self.showing_Timer = 0
